...
<authorship>
    ...
    <xsl:apply-templates select="ns1:coauthors/ns1:data[@tag='951']">
        <xsl:with-param name="viafID" select="$viafID" />
        <xsl:with-param name="viafType" select="$viafType" />
    </xsl:apply-templates>
    <xsl:apply-templates select="ns1:ISBNs/ns1:data">
        <xsl:with-param name="viafID" select="$viafID" />
        <xsl:with-param name="viafType" select="$viafType" />
    </xsl:apply-templates>
    ...
    <xsl:apply-templates select="ns1:titles/ns1:work">
        <xsl:with-param name="viafID" select="$viafID" />
        <xsl:with-param name="viafType" select="$viafType" />
    </xsl:apply-templates>
</authorship>
...
<xsl:template match="ns1:ISBNs/ns1:data" >
    <xsl:param name="viafID"/>
    <xsl:param name="viafType" />
    <xsl:param name="type" select="'isbn'" />
    <xsl:param name="sequence" select="position()" />
    <xsl:param name="exactText" select="ns1:text" />
    <xsl:apply-templates select="ns1:sources/ns1:s">
        <xsl:with-param name="viafID" select="$viafID" />
...
<xsl:template match="ns1:coauthors/ns1:data[@tag='951']" >
    <xsl:param name="viafID"/>
    <xsl:param name="viafType" />
    <xsl:param name="type" select="'corporate'" />
    <xsl:param name="sequence" select="position()" />
    <xsl:param name="exactText" select="ns1:text" />
    <xsl:apply-templates select="ns1:sources/ns1:s">
        <xsl:with-param name="viafID" select="$viafID" />
